...
L1 = function(survMod, type = "Margin", logScale = F, method = "Median"){
  ...
  uncensoredPiece = ifelse(!logScale,
                           sum(square(trueDeathTimes[as.logical(censorStatus)] - averageUncensored)),
                           sum(square(log(trueDeathTimes[as.logical(censorStatus)]) - log(averageUncensored))))
  L1Measure = switch(type,
                     ...
                     Margin = {
                       ...
                       marginPiece = ifelse(!logScale,
                                            ...
                                            sum(weights*(square(bestGuess - averageCensored))),
                                            sum(weights*(square(log(bestGuess) - log(averageCensored))))
                       )
