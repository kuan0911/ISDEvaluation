...
BayesianNet = function(training, testing){
  ...
  dataList = createTimeSplit(training,timesplit,includeNa = T)
  ...
  data$time = NULL
  start = hc(data,score='bic',whitelist = c('PREVTIMEPOINT','TIMEPOINT'),k=2)
  ...
  for(i in 1:length(dataList)) {
    ...
    #}
    #dag = hc(data,start=start,score='bic',k=1)
    #dag = cextend(dag, strict = TRUE, debug = FALSE)
    dag = structural.em(data, maximize = "hc",maximize.args = list(blacklist=c('TIMEPOINT','PREVTIMEPOINT')), fit = "mle",return.all = T,start = start, max.iter = 10, debug = FALSE)
    dagList[[i]] = dag 
    start = dag$dag
    #fit = bn.fit(dag, data, method='bayes')
    fitList[[i]] <- dag$fitted
