...
BayesianNet = function(training, testing, BayesianC1 = 1, timePoints = 0,debug = FALSE){
  ...
  for(iter in 1:1) {
    ...
    for(i in 1:numTimepoint) {
      ...
      data = data[rep(seq_len(nrow(data)), n.times),]
      # 
      # }
      ...
      #blackList = blacklistFunction(names(variableList[variableList==T&names(variableList)!='time']))
      dag = structural.em(data, maximize = "hc",maximize.args = list(restart=20,blacklist=NULL,whitelist=NULL,k=50), fit = "bayes",fit.args=list(iss=20),impute='bayes-lw',return.all = T,start = start$dag, max.iter = 3, debug = FALSE)
