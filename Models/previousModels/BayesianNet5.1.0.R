...
BayesianNet = function(training, testing){
  ...
  #timesplit2 = timesplit[seq(1, length(timesplit), 2)]
  numTimepoint2 = length(timesplit2)
  
  numTimepoint3 = 5
  timesplit3 = timeSplitFunction(numTimepoint = numTimepoint3, method = 'fixrate', data = training, debug=T)
  #timesplit3 = timesplit2[seq(1, length(timesplit2), 2)]
  numTimepoint3 = length(timesplit3)
  
  numTimepoint4 = 3
  timesplit4 = timeSplitFunction(numTimepoint = numTimepoint4, method = 'fixrate', data = training, debug=T)
  #timesplit4 = timesplit3[seq(1, length(timesplit3), 2)]
  numTimepoint4 = length(timesplit4)
  
  numTimepoint5 = 2
  timesplit5 = timeSplitFunction(numTimepoint = numTimepoint5, method = 'fixrate', data = training, debug=T)
  #timesplit5 = timesplit4[seq(1, length(timesplit4), 2)]
  numTimepoint5 = length(timesplit5)
  ...
  dataList5 = createTimeSplit(training,timesplit5)
  ...
  fitList5 <- vector("list", numTimepoint5)
  dagList5 <- vector("list", numTimepoint5)
  nbList5 <- vector("list", numTimepoint5)
  childrenList5 <- vector("list", numTimepoint5)
  ...
  for(i in 1:length(dataList5)) {
    ...
    nb_model = naiveBayes(as.factor(TIMEPOINT) ~., data=data)
    nbList5[[i]] <- nb_model
    childrenList5[[i]] <- children(fitList5[[i]],'TIMEPOINT')
    #print(mb(fitList[[i]],'TIMEPOINT'))
  }
  
  if(prior) {
    newFitList = imposePrior(dataList,fitList,numTimepoint)
    fitList = newFitList
    newFitList2 = imposePrior(dataList2,fitList2,numTimepoint2)
    fitList2 = newFitList2
    newFitList3 = imposePrior(dataList3,fitList3,numTimepoint3)
    fitList3 = newFitList3
    newFitList4 = imposePrior(dataList4,fitList4,numTimepoint4)
    fitList4 = newFitList4
    newFitList5 = imposePrior(dataList5,fitList5,numTimepoint5)
    fitList5 = newFitList5
  }
  ...
  print('start predict')
  #prediction
  survivalFunctionTesting1 = predictFunction(fitList,nbList,childrenList,testing,timesplit,method = queryMethod)
  survivalFunctionTraining1 = predictFunction(fitList,nbList,childrenList,training,timesplit,method = queryMethod)
  ...
  survivalFunctionTesting5 = predictFunction(fitList5,nbList5,childrenList5,testing,timesplit5,method = queryMethod)
  survivalFunctionTraining5 = predictFunction(fitList5,nbList5,childrenList5,training,timesplit5,method = queryMethod)
  
  survivalFunctionTesting1 = fixtime(survivalFunctionTesting1)
  survivalFunctionTraining1 = fixtime(survivalFunctionTraining1)
  survivalFunctionTesting2 = fixtime(survivalFunctionTesting2)
  survivalFunctionTraining2 = fixtime(survivalFunctionTraining2)
  survivalFunctionTesting3 = fixtime(survivalFunctionTesting3)
  survivalFunctionTraining3 = fixtime(survivalFunctionTraining3)
  survivalFunctionTesting4 = fixtime(survivalFunctionTesting4)
  survivalFunctionTraining4 = fixtime(survivalFunctionTraining4)
  survivalFunctionTesting5 = fixtime(survivalFunctionTesting5)
  survivalFunctionTraining5 = fixtime(survivalFunctionTraining5)

  
  survivalFunctionTesting = overlapCurve(survivalFunctionTesting1,survivalFunctionTesting2,survivalFunctionTesting3,survivalFunctionTesting4,survivalFunctionTesting5)
  testCurvesToReturn = smoothCurve(survivalFunctionTesting)
  ...
  survivalFunctionTraining = overlapCurve(survivalFunctionTraining1,survivalFunctionTraining2,survivalFunctionTraining3,survivalFunctionTraining4,survivalFunctionTraining5)
  trainingCurvesToReturn = smoothCurve(survivalFunctionTraining)
  ...
  for(i in 1:numTimepoint) {
    ...
    for(j in 1:nrow(testing)) {
      ...
      if(method == 'lw') {
        ...
        evidence = testing[j,]
        predicted = predict(tempFit, node="TIMEPOINT", evidence,method = "bayes-lw", prob = TRUE, n=10000)
        attr(predicted, "prob")
      ...
      if(i==1 & survivalFunction[i,j]<0.51){
        survivalFunction[i,j] = 0.51
...
smoothCurve <-function(survivalCurve) {
  ...
  for(i in 3:(nrow(survivalCurve)-2)) {
    for(j in 2:ncol(survivalCurve)) {
      newSurvivalCurve[i,j] = (survivalCurve[i-2,j]+survivalCurve[i-1,j]+survivalCurve[i,j]+survivalCurve[i+1,j]+survivalCurve[i+2,j])/5
    }
  }
...
overlapCurve <- function(curve1, curve2, curve3, curve4, curve5) {
  newCurve = curve1
  i2 = 1
  i3 = 1
  i4 = 1
  ...
  for(i in 2:(nrow(curve1)-1)) {
    if(curve1[i,1]>curve2[i2,1]&nrow(curve2)>i2) {i2 = i2+1}
    if(curve1[i,1]>curve3[i3,1]&nrow(curve3)>i3) {i3 = i3+1}
    if(curve1[i,1]>curve4[i4,1]&nrow(curve4)>i4) {i4 = i4+1}
    if(curve1[i,1]>curve5[i5,1]&nrow(curve5)>i5) {i5 = i5+1}
    
    for(j in 2:ncol(curve1)) {
      if(nrow(curve2)>i2&i2>1) {curve2_inter = curve2[i2-1,j]+(curve2[i2,j]-curve2[i2-1,j])*(curve1[i,1]-curve2[i2-1,1])/(curve2[i2,1]-curve2[i2-1,1])}
      else if(i2==1) {curve2_inter = 1+(curve2[i2,j]-1)*(curve1[i,1])/(curve2[i2,1])}
      else{curve2_inter = curve2[i2,j]}
      if(nrow(curve3)>i3&i3>1) {curve3_inter = curve3[i3-1,j]+(curve3[i3,j]-curve3[i3-1,j])*(curve1[i,1]-curve3[i3-1,1])/(curve3[i3,1]-curve3[i3-1,1])}
      else if(i3==1) {curve3_inter = 1+(curve3[i3,j]-1)*(curve1[i,1])/(curve3[i2,1])}
      else{curve3_inter = curve3[i3,j]}
      if(nrow(curve4)>i4&i4>1) {curve4_inter = curve4[i4-1,j]+(curve4[i4,j]-curve4[i4-1,j])*(curve1[i,1]-curve4[i4-1,1])/(curve4[i4,1]-curve4[i4-1,1])}
      else if(i4==1) {curve4_inter = 1+(curve4[i4,j]-1)*(curve1[i,1])/(curve4[i4,1])}
      else{curve4_inter = curve4[i4,j]}
      if(nrow(curve5)>i5&i5>1) {curve5_inter = curve5[i5-1,j]+(curve5[i5,j]-curve5[i5-1,j])*(curve1[i,1]-curve5[i5-1,1])/(curve5[i5,1]-curve5[i5-1,1])}
      else if(i5==1) {curve5_inter = 1+(curve5[i5,j]-1)*(curve1[i,1])/(curve5[i5,1])}
      else{curve5_inter = curve5[i5,j]}
      newCurve[i,j] = (curve5_inter+curve4_inter+curve3_inter+curve2_inter+curve1[i,j])/5
      #newCurve[i,j] = (curve5[i5,j]+curve4[i4,j]+curve3[i3,j]+curve2[i2,j]+curve1[i,j])/5
    }
  }
...
imposePrior <- function(dataList,fitList,numTimepoint) {
  ...
  for(i in 1:length(dataList)) {
    ...
    if(p != "Var1"){
      ...
      numData = nrow(data)
      weight = as.integer(numData/10)
      for(j in 1:length(timepoint1)) {
        timepoint1[j] = rbeta(1,(numTimepoint-1)*weight+timepoint1[j],1*weight+timepoint2[j])
