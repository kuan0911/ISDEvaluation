...
BayesianNet = function(training, testing){
  ...
  fitList1 <- vector("list", numTimepoint)
  fitList2 <- vector("list", numTimepoint)
  dagList <- vector("list", numTimepoint)
  dagList1 <- vector("list", numTimepoint)
  dagList2 <- vector("list", numTimepoint)
  ...
  data$time = NULL
  blacklistStart = blacklistFunctionStart(colnames(data))
  start = hc(data,score='bic')
  
  plotDag(start)
  
  blackList = blacklistFunction(colnames(dataList[[1]]))
  blackList1 = blackList$blackList1
  blackList2 = blackList$blackList2
  
  #whiteList = c('PREVTIMEPOINT','TIMEPOINT')
  for(i in 1:length(dataList)) {
    data_all = dataList[[i]]
    data = data_all[data_all[,'PREVTIMEPOINT']==0,]
    data_all$PREVTIMEPOINT = NULL
    ...
    dag1 = hc(data,blacklist=blackList1,score='bic')
    dag2 = hc(data,blacklist=blackList2,score='bic')
    dag = cextend(dag, strict = TRUE, debug = FALSE)
    dag1 = cextend(dag1, strict = TRUE, debug = FALSE)
    dag2 = cextend(dag2, strict = TRUE, debug = FALSE)
    dagList[[i]] = dag
    dagList1[[i]] = dag1 
    dagList2[[i]] = dag2
    fit = bn.fit(dag, data, method='bayes')
    fit1 = bn.fit(dag1, data, method='bayes')
    ...
    nbList[[i]] <- nb_model
    childrenList[[i]] <- children(fitList1[[i]],'TIMEPOINT')
    #plotDag(dag)
    ...
    for(i in 1:length(dataList)) {
      ...
      if(p != "Var1"){
        ...
        for(j in 1:length(timepoint1)) {
          timepoint1[j] = rbeta(1,(timesplitNumber-1)*weight*5+timepoint1[j]*5,1*weight*5+timepoint2[j]*5)
  ...
  print('start predict')
  #prediction
  survivalFunctionTesting = predictFunction(fitList,fitList1,fitList2,nbList,childrenList,testing,timesplit,method = queryMethod)
  survivalFunctionTraining = predictFunction(fitList,fitList1,fitList2,nbList,childrenList,training,timesplit,method = queryMethod)
...
predictFunction <- function(fitList,fitList1,fitList2,nbList,childrenList,testing,timesplit,method) {
  testing$time = NULL
  testing$delta = NULL
  numTimepoint = length(timesplit)
  numReturnNA = 0
  ...
  for(i in 1:numTimepoint) {
    ...
    for(j in 1:nrow(testing)) {
      ...
      if(method == 'lw') {
        ...
        prob = cpquery(tempFit, event = (TIMEPOINT == 0), evidence = evidence,method = 'lw')
        ...
        prob1 = 0
        prob2 = 0
        allEvidence = testing[j,]
        prob = 0
        for(k in 1:length(allEvidence)){
          #evidence = sample(allEvidence,5)
          evidence = allEvidence[k]
          #evidence = as.list(sample(as.list(testing[j,]), 10))
          predicted = predict(tempFit, node="TIMEPOINT", evidence,method = "bayes-lw", prob = TRUE, n=5000)
          #predicted1 = predict(tempFit1, node="TIMEPOINT", evidence,method = "bayes-lw", prob = TRUE, n=500)
          #predicted2 = predict(tempFit2, node="TIMEPOINT", evidence,method = "bayes-lw", prob = TRUE, n=500)
          prob = prob + attr(predicted, "prob")[1]
          #print(attr(predicted, "prob")[1])
          #prob1 = prob1 + attr(predicted1, "prob")[1]
          #prob2 = prob2 + attr(predicted2, "prob")[1]
        }
        #prob = (prob1+prob2)/2
        prob = prob/length(allEvidence)
        #print(prob)
      }else{
