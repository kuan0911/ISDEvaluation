...
BayesianNet = function(training, testing, BayesianC1 = 1, timePoints = 0,debug = FALSE){
  ...
  for(iter in 1:1) {
    ...
    start = structural.em(structureData, maximize = "hc",maximize.args = list(restart=15,blacklist=NULL,whitelist=NULL), fit = "bayes",fit.args=list(iss=20),impute='bayes-lw',return.all = T,start = NULL, max.iter = 2, debug = FALSE)
    ...
    for(i in 1:numTimepoint) {
      ...
      aliveData = data
      deadData = data
      aliveData$TIMEPOINT=factor(integer(nrow(aliveData)),levels = c('0','1'))
      deadData$TIMEPOINT=factor(rep(1,nrow(deadData)),levels = c('0','1'))
      n.times = c()
      for(k in 1:nrow(aliveData)) {
        id = aliveData[k,'id']
        ...
        n.times <- c(n.times,floor(weight))
      }
      aliveData = aliveData[rep(seq_len(nrow(aliveData)), n.times),]
      deadn.times = c()
      for(k in 1:nrow(deadData)) {
        id = deadData[k,'id']
        deadweight = 30-n.times[k]
        if(deadweight<=1) {
          deadweight = 1
        }
        deadn.times <- c(deadn.times,floor(deadweight))
      }
      deadData = deadData[rep(seq_len(nrow(deadData)), deadn.times),]
      data = rbind(aliveData,deadData)
      # 
      # }
      ...
      #blackList = blacklistFunction(names(variableList[variableList==T&names(variableList)!='time']))
      dag = structural.em(data, maximize = "hc",maximize.args = list(restart=20,blacklist=NULL,whitelist=NULL,k=60), fit = "bayes",fit.args=list(iss=20),impute='bayes-lw',return.all = T,start = start$dag, max.iter = 3, debug = FALSE)
      ...
      fit = bn.fit(dag$dag, data, method='bayes',iss=4)
  ...
  for(i in 1:length(dataList)) {
    ...
    for(j in 1:length(timepointProb)) {
      ...
      if(F) {
        ...
        survivalRate = 1 - (timepointNA[j]+timepoint2[j])/(timepoint1[j]+timepoint2[j]+timepointNATotal[j])
      }
  ...
  for(i in 1:length(dataList)) {
    for(node in children(fitList[[i]],'TIMEPOINT')) {
      ...
      for(k in 1:nrow(dataComplete)) {
        if(!is.na(dataComplete[k,'TIMEPOINT'])) {
      ...
      if(nrow(data[data$TIMEPOINT==1,])<dim(newcpt)[1]*1 | nrow(data[data$TIMEPOINT==0,])<dim(newcpt)[1]*1) {
        print('discard children')
