...
BayesianNet = function(training, testing){
  ...
  for(i in 1:length(dataList)) {
    ...
    dag = structural.em(data, maximize = "hc",maximize.args = list(blacklist=NULL), fit = "mle",impute='bayes-lw',return.all = T,start = NULL, max.iter = 5, debug = FALSE)
    dag = dag$dag
    dag = cextend(dag, strict = TRUE, debug = FALSE)
    dagList[[i]] = dag 
    start = dag
  ...
  for(i in 1:numTimepoint) {
    ...
    for(j in 1:nrow(testing)) {
      ...
      if(method == 'lw') {
        ...
        evidence = testing[j,]
        predicted = predict(tempFit, node="TIMEPOINT", evidence,method = "bayes-lw", prob = TRUE, n=1000)
        #attr(predicted, "prob")
