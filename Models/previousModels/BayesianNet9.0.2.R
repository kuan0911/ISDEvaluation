...
BayesianNet = function(training, testing, BayesianC1 = 1, timesplit = 0,debug = FALSE){
  ...
  for(i in 1:numTimepoint) {
    ...
    if(timesplit[i]>timesplitLess[iLess]) {iLess = iLess+1}
  ...
  if(prior) {
    fitList = timepointPrior(dataList,fitList,training,mod,timesplit)
  }
    ...
    if(weighted) {
      ...
      if(iter==1) {
        fitList = weightedLearning(dataList,dataListNA,fitList,timesplit,training,childrenList,first=T,mod)
      }else {
        fitList = weightedLearning(dataList,dataListNA,fitList,timesplit,training,childrenList,first=F,mod)
      }
    }
...
timepointPrior <- function(dataList,fitList,training,mod,timesplit) {
...
weightedLearning <- function(dataList,dataListNA,fitList,timesplit,training,childrenList,first,mod) {
  kmMod = prodlim(Surv(time,delta)~1, data = training)
