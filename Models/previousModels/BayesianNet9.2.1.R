...
BayesianNet = function(training, testing, BayesianC1 = 1, timePoints = 0,debug = FALSE){
  ...
  print('start predict')
  #prediction
  survivalFunctionTesting = predictFunction(fitList,nbList,childrenList,testing,originalTesting,timePoints,queryMethod,dataList,contFittingList,cont,variableList)
  survivalFunctionTraining = predictFunction(fitList,nbList,childrenList,training,originalTraining,timePoints,queryMethod,dataList,contFittingList,cont,variableList)
...
predictFunction <- function(fitList,nbList,childrenList,testing,originalTesting,timePoints,queryMethod,dataList,contFittingList,cont,variableList) {
  testing$time = NULL
  testing$delta = NULL
...
timepointPrior <- function(dataList,fitList,training,mod,timePoints) {
  ...
  for(i in 1:length(dataList)) {
    ...
    if(i>1){
      kmprob = predict(kmMod,timePoints[i])/predict(kmMod,timePoints[i-1])
    }else{
      kmprob = predict(kmMod,timePoints[i])
    }
