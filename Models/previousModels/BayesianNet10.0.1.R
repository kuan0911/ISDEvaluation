...
BayesianNet = function(training, testing, BayesianC1 = 1, timePoints = 0,debug = FALSE){
  ...
  for(iter in 1:1) {
    ...
    start = structural.em(structureData, maximize = "hc",maximize.args = list(restart=15,blacklist=NULL,whitelist=NULL,score='bic',k=1), fit = "bayes",fit.args=list(iss=20),impute='bayes-lw',return.all = T,start = NULL, max.iter = 2, debug = FALSE)
    
    if(fillAllMB) {
      print('fill all markov blanket')
      for(parent in parents(dag$dag,'TIMEPOINT')) {
        for(child in children(dag$dag,'TIMEPOINT')) {
          dag$dag = set.arc(dag$dag, parent, child)
        }
      }
    }
    ...
    for(i in 1:numTimepoint) {
      ...
      for(k in 1:nrow(data)) {
        ...
        #   if(data[k,'TIMEPOINT']==1&data[k,'PREVTIMEPOINT']==1){
        #     data[k,'TIMEPOINT'] = 0
        #   }
        if(!is.na(data[k,'PREVTIMEPOINT'])) {
      ...
      #blackList = blacklistFunction(names(variableList[variableList==T&names(variableList)!='time']))
      dag = structural.em(data, maximize = "hc",maximize.args = list(restart=50,blacklist=NULL,whitelist=NULL), fit = "bayes",fit.args=list(iss=20),impute='bayes-lw',return.all = T,start = start$dag, max.iter = 1, debug = FALSE)
      ...
      fit = bn.fit(dag$dag, data, method='bayes',iss=1)
    ...
    bnCurveList=NULL
    if(prior) {
      fitList = timepointPrior(dataList,fitList,prevFitList,originalTraining,mod,timePoints,kmMod,bnCurveList)
    }
    if(weighted) {
      fitList = weightedLearning(dataList,fitList,prevFitList,timePoints,originalTraining,mod,kmMod,bnCurveList)
    }
    prevFitList = fitList
  ...
  print('start predict')
  #prediction
  survivalFunctionTesting = predictFunction(fitList,fitListParents,nbList,childrenList,testing,originalTesting,timePoints,queryMethod,dataList,contFittingList,cont,variableList,kmMod)
  survivalFunctionTraining = predictFunction(fitList,fitListParents,nbList,childrenList,training,originalTraining,timePoints,queryMethod,dataList,contFittingList,cont,variableList,kmMod)
...
predictFunction <- function(fitList,fitListParents,nbList,childrenList,testing,originalTesting,timePoints,queryMethod,dataList,contFittingList,cont,variableList,kmMod) {
  testing$time = NULL
  testing$delta = NULL
    ...
    for(j in 1:length(timepointProb)) {
      ...
      if(T) {
        if(length(timepoint2)>1) {
          survivalRate = 1 - (timepoint2[j])/(timepoint1[j]+timepoint2[j]+0.5*timepointNACount[j])
        }else {
          survivalRate = 1 - (timepoint2+iss_dead)/(timepoint1+timepoint2+0.5*timepointNACount+iss_total)
        }
      }else{
        if(length(timepoint2)>1) {
          survivalRate = 1 - (timepointNA[j]+timepoint2[j])/(timepoint1[j]+timepoint2[j]+0.5*timepointNACount[j]+timepointNATotal[j])
        }else {
          survivalRate = 1 - (timepointNA+timepoint2)/(timepoint1+timepoint2+0.5*timepointNACount+timepointNATotal)
      ...
      timepointProb[j] = 1-(iss_dead/iss_total)
      #timepointProb[j] = 1-sum(timepoint2)/(sum(timepoint1)+sum(timepoint2)+0.5*sum(timepointNACount))
    }
